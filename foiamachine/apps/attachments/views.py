...
from apps.attachments.models import Government, Nation, Language,\
 Statute
...
class StatuteDetailView(RequestListView, SingleObjectMixin):
    ...
    def get_queryset(self):
        ...
        requests = reduce(operator.or_, [Request.objects.for_group_a('public').filter(attachments__slug=attachments.slug) for government in governments], Request.objects.none())
